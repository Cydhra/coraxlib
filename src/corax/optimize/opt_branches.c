  ...
  if (params->parallel_reduce_cb)
  {
    if (unlinked)
    {
      params->parallel_reduce_cb(params->parallel_context, df, 
                                 params->partition_count, PLL_REDUCE_SUM);
    ...
    {
      double d[2] = {*df, *ddf};
      params->parallel_reduce_cb(params->parallel_context, d, 2, PLL_REDUCE_SUM);
      *df = d[0];
      *ddf = d[1];
    }
  }
                                                                         ...
                                                                         int))
{
  ...
  {
    pllmod_set_error(PLL_ERROR_NOT_IMPLEMENTED,
  ...
  params.brlen_linkage     = (partition_count > 1) ?
                             brlen_linkage : PLL_BRLEN_LINKED;
  params.max_newton_iters  = 30;
