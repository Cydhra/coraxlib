...
static void algo_bestnode_list_copy_entry(
    pllmod_bestnode_list_t *best_node_list, size_t idx, const node_entry_t *src)
{
  ...
  if (treeinfo->brlen_linkage == CORAX_BRLEN_UNLINKED)
  {
    for (p = 0; p < treeinfo->partition_count; ++p)
    {
      if (treeinfo->partitions[p])
      {
        ...
        if (p_brlen < bl_min)
        {
          pllmod_treeinfo_set_branch_length_partition(
              treeinfo, node, p, bl_min);
          treeinfo->pmatrix_valid[p][pmatrix_index] = 0;
        }
        else if (p_brlen > bl_max)
        {
          pllmod_treeinfo_set_branch_length_partition(
              treeinfo, node, p, bl_max);
          treeinfo->pmatrix_valid[p][pmatrix_index] = 0;
        }
      }
    }
                              ...
                              const pllmod_search_params_t *params)
{
  ...
  while ((r_edge = regraft_nodes[j]) != NULL)
  {
    ...
    if (loglh > entry->lh)
    {
      ...
      pllmod_treeinfo_get_branch_length_all(
...
#ifndef PLLMOD_SEARCH_GREEDY_BLO
  if (tmp_topol) pllmod_treeinfo_destroy_topology(tmp_topol);
#endif
