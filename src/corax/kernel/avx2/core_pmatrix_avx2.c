...
CORAX_EXPORT
int corax_core_update_pmatrix_16x16_avx2(double ** pmatrix,
                                       unsigned int rate_cats,
                                       const double * rates,
                                       const double * branch_lengths,
                                       const unsigned int * matrix_indices,
                                       ...
                                       unsigned int count)
{
  ...
  expd = (double *)corax_aligned_alloc(16*sizeof(double), CORAX_ALIGNMENT_AVX);
  temp = (double *)corax_aligned_alloc(256*sizeof(double), CORAX_ALIGNMENT_AVX);
  ...
  for (n = 0; n < rate_cats; ++n)
  {
    ...
    if (!transposed[index])
    {
      /* allocate space for transposed eigenvectors and check that
         allocation succeeds */
      double * tran = (double *)corax_aligned_alloc(256*sizeof(double),
                                                  CORAX_ALIGNMENT_AVX);
      if (!tran)
      {
        corax_aligned_free(expd);
