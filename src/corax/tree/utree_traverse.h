 ...
 * `CORAX_TREE_TRAVERSE_POSTORDER `or `CORAX_TREE_TRAVERSE_PREORDER`. The callback
 ...
 * `corax_utree_create_operations`). The function will fail if `nullptr `is passed
 * in for `outbuffer`. If the behavior of this function is needed, but the
 * buffer is not needed, please use `corax_utree_traverse_apply`.
 *
 ...
 * size.
 *
 * @return `CORAX_SUCCESS` on a traversal without errors. `CORAX_FAILURE` otherwise.
 *
