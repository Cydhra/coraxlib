...
PLL_EXPORT pllmod_subst_model_t * pllmod_util_model_info_protein(const char * model_name)
{
  ...
  if (model_index >= 0)
    ...
    {
      pll_set_error(PLLMOD_UTIL_ERROR_MODEL_UNKNOWN, "Protein model not found: %s", model_name);
      return NULL;
    }
}
...
PLL_EXPORT pllmod_mixture_model_t * pllmod_util_model_info_protmix(const char * model_name)
{
  ...
  if (model_index >= 0)
    ...
    {
      pll_set_error(PLLMOD_UTIL_ERROR_MODEL_UNKNOWN, "Protein mixture model not found: %s", model_name);
      return NULL;
    }
}
