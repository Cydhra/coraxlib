...
static int modes[N_MODES] = {PLL_GAMMA_RATES_MEDIAN, PLL_GAMMA_RATES_MEAN};
...
int main(int argc, char *argv[])
{
  ...
  if (pll_compute_gamma_cats(invalid_alpha, 4, rate_cats, PLL_GAMMA_RATES_MEAN)
      == PLL_FAILURE)
  {
  ...
  for (k = 0; k < N_CATS; ++k)
  {
    ...
    int check_states = 1;
    check_states &=
        pll_set_tip_states(partition, 0, odd_map, "BAACDCGCDA--AEECFAAD");
    check_states &=
        pll_set_tip_states(partition, 1, odd_map, "CACCABGCBA--BDDGFCDA");
    check_states &=
        pll_set_tip_states(partition, 2, odd_map, "AG-C-CGCAG--CGFCFACC");
    check_states &=
        pll_set_tip_states(partition, 3, odd_map, "CGDCBDGCAA--AB-C-AAG");
    check_states &=
        pll_set_tip_states(partition, 4, odd_map, "CGACFFGCCA--AF-D-AAG");
